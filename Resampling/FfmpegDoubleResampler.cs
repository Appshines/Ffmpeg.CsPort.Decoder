...
{
	/// <summary>Ports libswresample's scalar planar-double Kaiser resampling path and its streaming buffers.</summary>
	internal sealed class FfmpegDoubleResampler
	{
		...
		private readonly double[][] inputBuffer;
		...
		public FfmpegDoubleResampler(int inputSampleRate, int outputSampleRate, int channels)
			: this(inputSampleRate, outputSampleRate, channels, 16)
		{
		}

		internal FfmpegDoubleResampler(int inputSampleRate, int outputSampleRate, int channels, int filterSize)
		{
			...
			filterBank = new double[filterAllocation * (phaseCount + 1)];
			...
			inputBuffer = new double[channels][];
			for (var channel = 0; channel < channels; channel++) inputBuffer[channel] = new double[MaximumBufferedSamples];
		}
		...
		public int Convert(double[][] output, int outputOffset, int outputCount, double[][] input, int inputOffset, int inputCount)
		{
		...
		private int InvertInitialBuffer(double[][] input, int inputOffset, int inputCount)
		{
			if (index >= 0) return 0;
		...
		private int Resample(double[][] output, int outputOffset, int outputCount, double[][] input, int inputOffset, int inputCount, out int consumed)
		{
		...
		private int ResampleCommon(double[] output, int outputOffset, double[] input, int inputOffset, int count, bool updateContext)
		{
		...
		private int ResampleLinear(double[] output, int outputOffset, double[] input, int inputOffset, int count, bool updateContext)
		{
			var currentIndex = index;
			var currentFraction = fraction;
			var sampleIndex = 0;
			...
			for (var outputIndex = 0; outputIndex < count; outputIndex++)
			{
				...
				value += (double)((nextValue - value) * inverseSourceIncrement * currentFraction);
		...
		private void BuildFilter(double factor, double kaiserBeta)
		{
			...
			for (var phase = 0; phase < phaseRows; phase++)
			{
				...
				for (var filter = 0; filter < filterLength; filter++) filterBank[phase * filterAllocation + filter] = (double)(table[filter] / normalization);
		...
		private bool ValidatePlanes(double[][] planes, int offset, int count)
		{
		...
		private void CopyPlanes(double[][] source, int sourceOffset, double[][] destination, int destinationOffset, int count)
		{
